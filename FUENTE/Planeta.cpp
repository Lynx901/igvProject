...
void Planeta::dibujar() {
  if(!explotado) {
    ...
    if(nombre == "pluton")   {year9 += 0.11; glRotatef(year9, 0, 1, 0);}
      ...
      if(nombre == "sol")             {
        igvTextura textSol("/texturas/sol.bmp");
        textSol.aplicar();
        gluSphere(qobj, 6,    50, 50);
      }else if(nombre == "mercurio")  {
        igvTextura textMerc("/texturas/mercurio.bmp");
        textMerc.aplicar();
        gluSphere(qobj, 1,    50, 50);
      }else if(nombre == "venus")     {
        igvTextura textVenus("/texturas/venus.bmp");
        textVenus.aplicar();
        gluSphere(qobj, 1.5,  50, 50);
      }else if(nombre == "tierra")    {
        igvTextura textTierra("/texturas/tierra.bmp");
        textTierra.aplicar();
        gluSphere(qobj, 2,    50, 50);
      }else if(nombre == "marte")     {
        igvTextura textMarte("/texturas/marte.bmp");
        textMarte.aplicar();
        gluSphere(qobj, 1.5,  50, 50);
      }else if(nombre == "jupiter")   {
        igvTextura textJupiter("/texturas/jupiter.bmp");
        textJupiter.aplicar();
        gluSphere(qobj, 4.5,  50, 50);
      }else if(nombre == "saturno")   {
        igvTextura textSaturno("/texturas/saturno.bmp");
        textSaturno.aplicar();
        gluSphere(qobj, 4,    50, 50);
      }else if(nombre == "urano")     {
        igvTextura textUrano("/texturas/urano.bmp");
        textUrano.aplicar();
        gluSphere(qobj, 3.5,  50, 50);
      }else if(nombre == "neptuno")   {
        igvTextura textNeptuno("/texturas/neptuno.bmp");
        textNeptuno.aplicar();
